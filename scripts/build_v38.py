#!/usr/bin/env python3
"""
build_v38.py  —  v10 design, all fixes -> v38.zip

Fixes over v37:
  1. /app URL: copy app.html → app/index.html so getquran.co/app works
     (Cloudflare Pages serves index.html at directory URLs, not .html files)
  2. Nav "Get the App" link: replace href="#app" → href="/app" on all pages
     (was anchoring to homepage #app section instead of going to /app page)
...
OUT_ZIP  = os.path.expanduser('~/Downloads/getquran_cloudflare_deploy_v38.zip')
...
with zipfile.ZipFile(IN_ZIP, 'r') as zin, \
    ...
    for item in zin.infolist():
        ...
        if m:
            ...
            zout.writestr(item, raw)
            # Fix 1: also write app.html as app/index.html so /app URL works on Cloudflare Pages
            if item.filename == 'app.html':
                zout.writestr('app/index.html', raw)
                print('Wrote app/index.html (so /app URL works)')
