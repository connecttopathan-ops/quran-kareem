#!/usr/bin/env python3
"""
build_v39.py  —  v10 design, all fixes -> v39.zip

Fixes over v38:
  1. /app URL: use _redirects rewrite (/app /app.html 200) instead of
     app/index.html — app.html itself contains a JS redirect, so the
     app/index.html approach in v38 caused a redirect loop. A 200 rewrite
     in _redirects serves app.html content at /app without any redirect.
  2. Nav "Get the App" link: href="#app" → href="/app" (retained from v38)
...
OUT_ZIP  = os.path.expanduser('~/Downloads/getquran_cloudflare_deploy_v39.zip')
...
with zipfile.ZipFile(IN_ZIP, 'r') as zin, \
    ...
    for item in zin.infolist():
        ...
        if item.filename == '_redirects':
            # Append our /app rewrite rule to v10's existing redirects.
            # Status 200 = Cloudflare Pages "rewrite": serves app.html content
            # at /app URL with no HTTP redirect (avoids the JS-redirect loop).
            existing = raw.decode('utf-8').rstrip()
            merged = existing + '\n/app /app.html 200\n'
            zout.writestr('_redirects', merged.encode('utf-8'))
            print('Wrote _redirects (appended /app /app.html 200 rewrite)')
            redirects_done = True
            continue
    ...
    # If v10.zip had no _redirects, write one from scratch
    if not redirects_done:
        zout.writestr('_redirects', b'/app /app.html 200\n')
        print('Wrote _redirects (created, /app /app.html 200 rewrite)')
