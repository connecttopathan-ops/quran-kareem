#!/usr/bin/env python3
"""
build_v44.py  —  v10 design, all fixes -> v44.zip

Fixes over v43:
  1. favicon.ico at root: Google's crawler requests /favicon.ico directly.
     Generates a valid PNG-in-ICO file from icon.png so Google Search
     picks up the correct favicon faster.

All previous fixes retained:
  - SEO slug URLs (/surah/al-baqarah/ etc.)
  ...
  - Circular black favicon (SVG + PNG)
...
OUT_ZIP  = os.path.expanduser('~/Downloads/getquran_cloudflare_deploy_v44.zip')
...
def make_favicon_ico():
    """Create a valid PNG-in-ICO file from icon.png.
    Google's crawler hits /favicon.ico directly — this ensures it resolves."""
    import struct
    with open(ICON_PATH, 'rb') as f:
        png_bytes = f.read()
    ...
    # ICONDIRENTRY: width=0 (>=256), height=0, colorCount=0, reserved=0,
    #               planes=1, bitCount=32, sizeInBytes, offsetInBytes
    entry  = struct.pack('<BBBBHHII', 0, 0, 0, 0, 1, 32, img_size, img_offset)
    return header + entry + png_bytes

# Inject SVG favicon + fallback PNG + favicon.ico, replace old PNG-only favicon tags
FAVICON_TAGS = (
...
with zipfile.ZipFile(IN_ZIP, 'r') as zin, \
    ...
    # Write favicon.ico — Google's crawler hits /favicon.ico directly
    favicon_ico = make_favicon_ico()
    zout.writestr('favicon.ico', favicon_ico)
    print(f'Wrote favicon.ico ({len(favicon_ico)//1024} KB, PNG-in-ICO)')
    ...
    for item in zin.infolist():
        ...
        if item.filename.startswith('translations/'):
            continue
        if item.filename in ('sitemap.xml', 'robots.txt', 'favicon.svg', 'favicon.ico',
                             'icon.png', 'app/index.html', 'app.html'):
